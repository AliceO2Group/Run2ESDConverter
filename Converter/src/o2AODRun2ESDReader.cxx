...
WorkflowSpec defineDataProcessing(ConfigContext const &specs) {
  WorkflowSpec workflow{
      ...
      DataProcessorSpec{
          "run2-esd-converter",
          {},
          {OutputSpec{"ESD", "TRACKPAR"}, OutputSpec{"ESD", "TRACKPARCOV"},
           OutputSpec{"ESD", "TRACKEXTRA"}, OutputSpec{"ESD", "CALO"},
           OutputSpec{"ESD", "MUON"}, OutputSpec{"ESD", "VZERO"}},
          run2::Run2ESDConversionHelpers::getESDConverter(),
          {ConfigParamSpec{"esd-file",
                           VariantType::String,
                           "run2Esd.root",
                           {"the Run 2 ESD file to convert"}}}},
      ...
      DataProcessorSpec{
          ...
          {// Dangling inputs of type AOD will be automatically picked up
           ...
           InputSpec{"Calo", "ESD", "CALO"}, InputSpec{"Muon", "ESD", "MUON"},
          ...
          AlgorithmSpec{[](InitContext &setup) {
            return [](ProcessingContext &ctx) {
              ...
              if (table->num_rows() == 0) {
                LOG(ERROR) << "Arrow table is TRACKPAR is empty"
                           << table->num_rows();
              }
              if (table->num_columns() != 8) {
                LOG(ERROR) << "Wrong number of columns for the arrow table"
                           << table->num_columns();
              }
              auto source = std::make_unique<ROOT::RDF::RArrowDS>(
                  s->asArrowTable(), std::vector<std::string>{});

              ROOT::RDataFrame rdf(std::move(source));
              auto h1 =
                  rdf.Define("pt", "TMath::Abs(1/fSigned1Pt)").Histo1D("pt");
              h1->Draw();
            };
          }}}};
  return workflow;
